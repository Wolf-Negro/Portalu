  ...
  return (
    <div className="flex h-screen animate-fade-in" style={{ maxHeight: "100vh" }}>
      ...
      <div className="w-72 flex flex-col flex-shrink-0"
        style={{ borderRight: "1px solid rgba(114,85,180,0.15)", background: "rgba(14,14,20,0.8)" }}>
        <div className="p-4 flex-shrink-0" style={{ borderBottom: "1px solid rgba(114,85,180,0.12)" }}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-semibold text-sm" style={{ color: "#e9e8e6" }}>WhatsApp</h2>
          </div>
          <div className="relative">
            <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2" style={{ color: "#5a5575" }} />
            <input
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Buscar conversaciones..."
              className="w-full pl-8 pr-3 py-2 rounded-lg text-xs outline-none"
              style={{ background: "rgba(22,22,42,0.8)", border: "1px solid rgba(114,85,180,0.2)", color: "#e9e8e6" }}
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {filtered.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-xs" style={{ color: "#5a5575" }}>Sin conversaciones</p>
            </div>
          ) : (
            filtered.map((conv) => {
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate" style={{ color: "#e9e8e6" }}>{conv.contact}</p>
                      {lastMsg && (
                        <p className="text-xs truncate mt-0.5" style={{ color: "#5a5575" }}>{lastMsg.content}</p>
                      )}
                    </div>
        ...
        {selectedConv ? (
          <>
            ...
            <div className="px-5 py-3.5 flex-shrink-0 flex items-center justify-between"
              style={{ borderBottom: "1px solid rgba(114,85,180,0.15)", background: "rgba(22,22,42,0.6)" }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: "#e9e8e6" }}>{selectedConv.contact}</p>
                  {selectedConv.phone && (
                    <p className="text-xs flex items-center gap-1" style={{ color: "#5a5575" }}>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              {messages.length === 0 ? (
                <div className="flex items-center justify-center h-full">
                  <p className="text-sm" style={{ color: "#5a5575" }}>Sin mensajes aún</p>
                </div>
              ) : (
                messages.map((msg) => (
                  <div
            ...
            <div className="p-4 flex-shrink-0"
              style={{ borderTop: "1px solid rgba(114,85,180,0.15)", background: "rgba(22,22,42,0.6)" }}>
              <div className="flex gap-2">
                <input
                  value={newMsg}
                  ...
                  style={{ background: "rgba(26,26,46,0.8)", border: "1px solid rgba(114,85,180,0.25)", color: "#e9e8e6" }}
                />
                <button
                  onClick={handleSend}
                  ...
                  style={{
                    background: newMsg.trim() && !sending ? "linear-gradient(135deg, #2b096f, #7255b4)" : "rgba(114,85,180,0.2)",
                    color: "#e9e8e6",
                  }}
                >
                  <Send size={16} />
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <MessageSquare size={32} className="mx-auto mb-3" style={{ color: "#5a5575" }} />
              <p className="text-sm" style={{ color: "#5a5575" }}>Selecciona una conversación</p>
            </div>
          </div>
        )}
      </div>
